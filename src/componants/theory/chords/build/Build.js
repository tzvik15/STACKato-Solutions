...
const Build = () => {
  ...
  return choice.root === false && choice.inversion === false ? (
    <>
      <p className="infoS" id="buildFirstChoice">
        In this section you will be presented with a chord that is missing a
        note, and the type of chord to be built. Your objective is to select the
        note that will complete the chord.{" "}
      </p>
      <h4 className="infoOtherChords" id="buildChoice">
        Would you like to practice root position chords, or include inversions
        as well?
      </h4>
      <div id="build1Btn">
        <button className="btn-hover colorRoot" onClick={rooter}>
          Root Positions!
        </button>
        <button className="btn-hover colorInversions" onClick={inversionator}>
          Inversions!
        </button>
        <Link to="/theory/chords">
          <button className="btn-hover colorHome  " id="build1Home">
            {" "}
            HOME!
          </button>
        </Link>
    ...
    <>
      <div id="inversionDivAll">
        <h1 className="title">Chords with Inversions</h1>
        <h4 className="infoS" id="someId">
          Please complete the following chord.
        </h4>
        <div id="testInvDiv">
          <p className="infoOtherChords">Chord name: {inverRand.name}</p>
          <br />
          <br />
          <img
            src={inverRand.pic}
            alt="inverted chord with missing note"
            id="missingInvPic"
          />
          <div id="invMapBtn">
            <h4 className="infoS">
              Please select the note that would complete the chord
            </h4>
            <div id="mapBtns">
              {inverChoices.map((item) => (
                <button onClick={selected2} className="noteBtn">
                  <img className="imgBtn" src={item} alt="note" />
                </button>
              ))}
            </div>
            <div id="buildInvEnd">
              <h1 className="infoOtherChords" id="buildInvYes">
                Correct Answers: {count.yes}
              </h1>
              <h1 className="infoOtherChords" id="buildInvNo">
                Incorrect Answers: {count.no}
              </h1>
              <Link to="/theory/chords">
                <button className="btn-hover colorHome  " id="buildInvHome">
                  {" "}
                  HOME!
                </button>
              </Link>
    ...
    <>
      <h1 className="title" id="buildRTitle">
        Chords in Root Position
      </h1>
      <h4 className="infoS" id="buildRDirect">
        Please complete the following chord.
      </h4>
      <div>
        <p className="infoOtherChords">Chord name: {rootRand.name}</p>
        <br />
        <br />
        <div id="buildRBtns">
          <img src={rootRand.pic} alt="chord with missing note" />
          <h4 className="infoS" id="buildRDirections">
            Please select the note that would complete the chord
          </h4>
          <br />
        <div id="buildRMapBtn">
          {rootChoices.map((item) => (
            <button onClick={selected1} className="noteBtn">
              <img className="imgBtn" src={item} alt="note" />
            </button>
          ))}
</div>
          <div id="buildREnd">
            <h1 className="infoOtherChords">Correct Answers: {count.yes}</h1>
            <h1 className="infoOtherChords">Incorrect Answers: {count.no}</h1>
            <Link to="/theory/chords">
              <button className="btn-hover colorHome  " id="buildRHome"> HOME!</button>
            </Link>
          </div>
        </div>
      </div>
