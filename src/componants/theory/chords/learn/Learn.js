...
const Learn =()=> {
    ...
    return (
        ...
        <h2 className ="titleL">Triad types, and how to differentiate between them</h2>
        <p className ="infoL">There are four (4) types of triads in Western Classical Music. Each one of these types sounds differently when played. The type of triad depends on the type and order of thirds. As mentioned, triads are a stack of 2 thirds. Both of those thirds can be either major ("M"), or minor ("m"). 
        ...
        <p className ="infoL">At this point, it's important to point out that while there are many theoretical seventh chords, in Western Classical Music Common Practice Period (the time span roughly 1600-1910, from which we derive most of our music theory practices), several of those chords are not present. While they DO exist theoretically and are used in other styles of music (jazz for example), this application will only discuss the chords we commonly use.</p>
        <h4 className ="titleL">Major Triad Septachords</h4>
        <p className ="infoL">There are 2 types of septachords commonly used that are based on a major triad. They are created by adding a major or minor seventh to the major chord. These resulting septachords are named "Major Major" and "Major Minor" respectively. On a root note of "C", they would look like this: C-E-G-B (Major Major), C-E-G-Bb (major minor). </p>
        <p className ="infoL">The Major Major chord is sometimes just called a Major 7th chord. It can by spelled either XM7 (CM7, DbM7, G#M7, etc.), or XMM (CMM, DbMM, G#MM, etc.) Sometimes, instead of the "M", the chord is spelled with a triangle: X∆7 (C∆7, Db∆7, G#∆7, etc.) </p>
        <p className ="infoL">The Major Minor chord is sometimes called a Dominant Seventh chord. It is comprised of a major triad and a minor seventh, so on a root note of C it will have the following notes: C-E-G-Bb. It can be spelled either XMm7 (CMm7, DbMm7, G#Mm7, etc.), or just X7 (C7, Db7, G#7, etc.)</p>
        <h4 className ="titleL">Minor Triad Septachords</h4>
        <p className ="infoL">While the exact same theoretical logic guiding the creation of the major triad septachords holds true for minor triads, in Western Classical Music we only use one minor septachord: the Minor Minor seventh chord. This septachord is comprised of a minor triad and a minor seventh above the root. On a root note of C, it will look like this: C-Eb-G-Bb.This type of septachord is sometimes called Minor Seventh. It is labelled Xm7 (Cm7, Dbm7, G#m7, etc.), or sometimes X-7 (C-7, Db-7, G#-7, etc.)</p>
        <h4 className ="titleL">Diminished Triad Septachords</h4>
        <p className ="infoL">There are two (2) types of diminished septachords we use in Western Classical Music based on the diminished triad. The first septachord is called a Half-Diminished seventh chord and is comprised of a diminished triad with a minor seventh added above the root. From a root note of C: C-Eb-Gb-Bb. This chord is sometimes called a Minor Seventh Flat (b) Five chord, and is spelled Xø7 (Cø7, Dbø7, G#ø7, etc.), or sometimes X-7b5 (C-7b5, Db-7b5, G#-7b5, etc.)</p>
        <p className ="infoL">The other chord based on a diminished triad has a DIMINISHED seventh interval added to it. On a root of C, it is spelled: C-Eb-Gb-Bbb, and is called a Fully Diminished chord, or sometimes a diminished seventh chord. It is spelled Xo7 (Co7, Dbo7, G#o7, etc.), or Xdim7 (Cdim7, Dbdim7, G#dim7, etc.).</p>
        <br></br>
        <p className ="infoL">In the Common Practice Period, we don't use seventh chords based on an augmented triad, though they do exist in other music styles and in theoretical work.</p>
        <p className ="titleL">Click the buttons below to listen to the various septachords.</p>
...
<div>
        ...
        <p className ="infoL">The second inversion of a septachord is named "terz-quart". In this inversion, the bottom note of the chord is the third note of the root position, or the fifth. (C-E-G-B ---> G-B-C-E)</p>
        <p className ="infoL">The third inversion of a septachord is named "secoond". In this inversion, the bottom note of the chord is the 4th note in the root position, or the seventh. (C-E-G-B ---> B-C-E-G)</p>
        <p className ="infoL">While all the inversions have a latin name (quint-sext, etc.), it is completely acceptable to call them by their inversion number, for example C Major Major, 2nd inversion, etc.)</p>
        ...
        <p className ="infoL">The inversions are spelled with numbers by the name of the chord. Root position: <img src={seven} alt="7" className="numberPicL"/>, 1st inversion: <img src={sixFive} alt="6-5" className="numberPicL"/>, 2nd inversion: <img src={threeFour} alt="3-4" className="numberPicL"/>, and 3rd inversion: <img src={two} alt="2" className="numberPicL"/> Note that when spelling a third inversion, sometimes the number "4" is omitted, and only the "2" appears. </p>
        <p className ="infoL">Click the buttons below to listen to the various septachords and inversions.</p>
        <div className="sounds">
        ...
        <p className ="infoL">As a triad has three (3) notes, there are three ways in which they can be ordered. The root position (sometimes referred to as "base" position, or not given a name (for example when you encounter the term "major triad/chord" it means root form)).<br></br>
         ...
         <br></br>
        <p className ="infoL">Root position triads are spelled either just with the base name ("M", "m", etc.), or with the numbers <img src={root} alt="3-5" className="numberPicL"/> next to the root name (this is rarely done).
            1st inversions are sometimes called "sextachords", and are spelled with the number "6" next to their name (M6, m6, etc.). <br></br>
            2nd inversions are sometimes called "quart-sextachords", and are spelled with the numbers <img src={quart} alt="4-6" className="numberPicL"/> attached to the name. The numbering of the inversions (both in writing and in name) are references to the intervalic content of the chord, in relation to the lowest note. <br></br>
            ...
            <p className ="infoL">Each one of the inverted chords has a different sound. To listen to the different inversions, press the buttons below.</p>
            <div className="sounds">
